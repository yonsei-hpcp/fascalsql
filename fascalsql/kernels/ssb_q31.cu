// SSB Q3.1 -- FaScalSQL optimized kernel (ODZC + AFP + CQO)
// Refactored: uses shared macros and CPU predicate helpers.
// Dimensions: customer (c_region=ASIA -> c_nation_enc), supplier (s_region=ASIA -> s_nation_enc)
// Date filter: lo_orderdate BETWEEN 19920101 AND 19971231 (replaces date dim join)
// GROUP BY: c_nation_enc, s_nation_enc, year (inline lo_orderdate/10000-1992)
...
__global__ void ssb_q31_kernel(
    ...
    unsigned long long *aggtable)
{
  ...
  int c_nation_enc[IPT], s_nation_enc[IPT];
  ...
  if (FASCAL_ANY_ALIVE(IPT)) do {
    ...
    // GROUP BY: c_nation_enc + s_nation_enc*25 + year*625
    #pragma unroll
    for (int i = 0; i < IPT; ++i)
      if ((threadIdx.x + BT * i) < num_tile_items && selection_flags[i]) {
        int gk = c_nation_enc[i] + s_nation_enc[i] * 25 + (lo_orderdate[i] / 10000 - 1992) * 625;
    ...
    const uint32_t *d_prefilter, const uint8_t *d_tile_summary)
{
  ...
  int key[IPT], region[IPT], payload[IPT];

  BlockLoadSelect<int, BT, IPT>((int*)(d_c_custkey + tile_offset), key, selection_flags, num_tile_items);
  BlockLoadSelect<int, BT, IPT>((int*)(d_c_region + tile_offset), region, selection_flags, num_tile_items);
  BlockPredAndEQ<int, BT, IPT>(region, 2, selection_flags, num_tile_items);
  BlockLoadSelect<int, BT, IPT>((int*)(d_c_nation_enc + tile_offset), payload, selection_flags, num_tile_items);
    ...
    const uint32_t *d_prefilter, const uint8_t *d_tile_summary)
{
  ...
  BlockLoadSelect<int, BT, IPT>((int*)(d_s_region + tile_offset), region, selection_flags, num_tile_items);
  BlockPredAndEQ<int, BT, IPT>(region, 2, selection_flags, num_tile_items);
  BlockLoadSelect<int, BT, IPT>((int*)(d_s_nation_enc + tile_offset), payload, selection_flags, num_tile_items);
...
int main(int argc, char** argv) {
  ...
  int cust_qual = (int)(fascal_estimate_eq_sel(h_c_region, num_cust, 2) * num_cust);
  int supp_qual = (int)(fascal_estimate_eq_sel(h_s_region, num_supp, 2) * num_supp);
  ...
  ODZC_DIM(c_region, h_c_region, num_cust, "c_region")
  ODZC_DIM(c_nation_enc, h_c_nation_enc, num_cust, "c_nation_enc")
  ODZC_DIM(s_suppkey, h_s_suppkey, num_supp, "s_suppkey")
  ODZC_DIM(s_region, h_s_region, num_supp, "s_region")
  ODZC_DIM(s_nation_enc, h_s_nation_enc, num_supp, "s_nation_enc")
  #undef ODZC_DIM

  CUDA_CHECK(cudaMemcpyToSymbol(d_pred_flags, h_pred_flags, sizeof(h_pred_flags)));

  // Aggregation table: 25 nations * 25 nations * 10 years = 6250 groups
  const size_t num_groups = 25ULL * 25 * 10;
  ...
  // CPU prefilter for build: customer (c_region == 2)
  fascal_cpu_prefilter_run(num_cust, h_cust_pf, h_cust_ts,
      [&](int off, int cnt) {
          ...
          for (int i = 0; i < cnt; ++i) { int idx = off + i;
            if (h_c_region[idx] == 2) h_cust_pf[idx >> 5] |= (1u << (idx & 31)); }
      });

  // CPU prefilter for build: supplier (s_region == 2)
  fascal_cpu_prefilter_run(num_supp, h_supp_pf, h_supp_ts,
      [&](int off, int cnt) {
          uint32_t ws = (uint32_t)off >> 5, we = ((uint32_t)(off + cnt) + 31) >> 5;
          memset(&h_supp_pf[ws], 0, (we - ws) * sizeof(uint32_t));
          for (int i = 0; i < cnt; ++i) { int idx = off + i;
            if (h_s_region[idx] == 2) h_supp_pf[idx >> 5] |= (1u << (idx & 31)); }
      });

  // Pipeline setup
  int *h_data[] = {h_lo_custkey, h_lo_suppkey, h_lo_orderdate, h_lo_revenue};
  // CPU-side BF mirrors for AFP probes
  fascal::runtime::BloomFilter h_bf_cust, h_bf_supp;
  h_bf_cust.bits = nullptr; h_bf_supp.bits = nullptr; // populated after build
  ...
  auto run_kernel = [&]() {
    ...
    auto launch = [&](int nt, int bo, int nb, cudaStream_t st) {
      ssb_q31_kernel<BLOCK_THREADS, ITEMS_PER_THREAD><<<nb, BLOCK_THREADS, 0, st>>>(
  ...
  odzc->unregister_column(mc_c_nation_enc);
  odzc->unregister_column(mc_s_suppkey); odzc->unregister_column(mc_s_region);
  odzc->unregister_column(mc_s_nation_enc);
