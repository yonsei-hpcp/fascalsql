// SSB Q2.1 -- FaScalSQL optimized kernel (ODZC + AFP + CQO)
// Refactored: uses shared macros and CPU predicate helpers.
// Part dim: p_category=1 (MFGR#12), payload=p_brand1. Supplier dim: s_region=1 (AMERICA).
...
__global__ void ssb_q21_kernel(
...
// =============================================================================
// Build kernel: part dimension (p_category=1, payload=p_brand1)
// =============================================================================
template <int BT, int IPT>
__global__ void build_part_kernel(
    int num_tuples,
    const int *__restrict__ d_p_partkey, const int *__restrict__ d_p_category,
    const int *__restrict__ d_p_brand1,
    ...
    const uint32_t *d_prefilter, const uint8_t *d_tile_summary)
{
  ...
  int pk[IPT], cat[IPT], br[IPT];
  BlockLoadSelect<int, BT, IPT>((int*)(d_p_partkey + tile_offset), pk, selection_flags, num_tile_items);
  BlockLoadSelect<int, BT, IPT>((int*)(d_p_category + tile_offset), cat, selection_flags, num_tile_items);
  BlockPredAndEQ<int, BT, IPT>(cat, 1, selection_flags, num_tile_items);
...
// =============================================================================
// Build kernel: supplier dimension (s_region=1, existence HT)
// =============================================================================
template <int BT, int IPT>
    ...
    const uint32_t *d_prefilter, const uint8_t *d_tile_summary)
{
  ...
  BlockPredAndEQ<int, BT, IPT>(sr, 1, selection_flags, num_tile_items);
...
static void cpu_prefilter_part(int *h_pk, int *h_cat, int nt, uint32_t *bm, uint8_t *ts) {
  fascal_cpu_prefilter_run(nt, bm, ts, [&](int off, int cnt) {
    ...
    for (int i = 0; i < cnt; ++i) { int idx = off + i;
      if (h_cat[idx] == 1) bm[(uint32_t)idx >> 5] |= (1u << ((uint32_t)idx & 31));
    }
  });
}
...
static void cpu_prefilter_supp(int *h_sk, int *h_sr, int nt, uint32_t *bm, uint8_t *ts) {
  fascal_cpu_prefilter_run(nt, bm, ts, [&](int off, int cnt) {
    ...
    memset(&bm[ws], 0, (we - ws) * sizeof(uint32_t));
    for (int i = 0; i < cnt; ++i) { int idx = off + i;
      if (h_sr[idx] == 1) bm[(uint32_t)idx >> 5] |= (1u << ((uint32_t)idx & 31));
    }
  });
}
...
int main(int argc, char** argv) {
  ...
  snprintf(path, sizeof(path), "%s/p_category.bin", data_dir);
  int *h_p_category = load_binary_column_auto(path, &num_part);
  if (!h_p_category) { fprintf(stderr, "Failed to load p_category\n"); return 1; }
  ...
  int part_qual = (int)(fascal_estimate_eq_sel(h_p_category, num_part, 1) * num_part);
  ...
  ODZC_DIM(p_category, h_p_category, num_part, "p_category")
  ...
  auto run_kernel = [&]() {
    ...
    auto launch = [&](int nt, int bo, int nb, cudaStream_t st) {
      ssb_q21_kernel<BLOCK_THREADS, ITEMS_PER_THREAD><<<nb, BLOCK_THREADS, 0, st>>>(
  ...
  // Build part
  cpu_prefilter_part(h_p_partkey, h_p_category, num_part, h_part_pf, h_part_ts);
  ...
  odzc->unregister_column(mc_p_partkey);    odzc->unregister_column(mc_p_category);
  odzc->unregister_column(mc_p_brand1);
