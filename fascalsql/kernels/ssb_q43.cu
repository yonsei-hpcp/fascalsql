// SSB Q4.3 -- FaScalSQL optimized kernel (ODZC + AFP + CQO)
// Refactored: uses shared macros and CPU predicate helpers.
// Dims: customer(no pred), supplier(s_nation=UNITED STATES(23), payload=s_city_enc), part(p_category=MFGR#14(3), payload=p_brand1)
// Date: 1997-1998, GROUP BY d_year(inline) x s_city_enc x p_brand1, AGG SUM(lo_revenue - lo_supplycost)
...
__global__ void ssb_q43_kernel(
    ...
    unsigned long long *aggtable)
{
  ...
  int lo_revenue[IPT], lo_supplycost[IPT], s_city_enc[IPT], p_brand1[IPT];

  if (FASCAL_ANY_ALIVE(IPT)) do {
    // Pred 0: lo_orderdate BETWEEN 19970101 AND 19981231
    if (d_pred_flags[0] == 0) {
      BlockLoadSelect<int, BT, IPT>(d_lo_orderdate + tile_offset, lo_orderdate, selection_flags, num_tile_items);
      BlockPredAndGTE<int, BT, IPT>(lo_orderdate, 19970101, selection_flags, num_tile_items);
    ...
    // HT joins
    BlockJoinProbeDirect<BT, IPT>(lo_custkey, selection_flags, num_tile_items, ht_customer, ht_size_customer, 1);
    BlockJoinProbePayloadDirect<BT, IPT>(lo_suppkey, selection_flags, num_tile_items, ht_supplier, ht_size_supplier, 1, s_city_enc);
    BlockJoinProbePayloadDirect<BT, IPT>(lo_partkey, selection_flags, num_tile_items, ht_part, ht_size_part, 1, p_brand1);
    ...
    // GROUP BY year(10) x city(250) x brand(1000) -> profit
    #pragma unroll
    for (int i = 0; i < IPT; ++i)
      if ((threadIdx.x + BT * i) < num_tile_items && selection_flags[i]) {
        int gk = (lo_orderdate[i] / 10000 - 1992) + s_city_enc[i] * 10 + p_brand1[i] * 2500;
...
static void cpu_predicate(int *h_data[], int offset, int cnt, uint32_t *bitmap) {
  ...
  if (h_pred_flags[0] == 1)
    fascal::cpu_pred::avx2_range_pass(bm, bs, lo_orderdate, offset, cnt, 19970101, 19981231, true, true);
}
...
// customer: no predicate, no payload (pass-through)
template <int BT, int IPT>
...
// supplier: s_nation == 23 (UNITED STATES), payload = s_city_enc
template <int BT, int IPT>
__global__ void build_supplier(
    int num_tuples,
    const int *__restrict__ d_s_suppkey, const int *__restrict__ d_s_nation, const int *__restrict__ d_s_city_enc,
    HtEntry *d_ht, uint32_t ht_size,
    uint64_t *__restrict__ d_bf_bits, uint32_t bf_size_bits, uint8_t num_hashes,
    const uint32_t *d_prefilter, const uint8_t *d_tile_summary)
{
  FASCAL_BUILD_KERNEL_PROLOGUE(BT, IPT);
  int k[IPT], n[IPT], c[IPT];
  BlockLoadSelect<int, BT, IPT>((int*)(d_s_suppkey + tile_offset), k, selection_flags, num_tile_items);
  BlockLoadSelect<int, BT, IPT>((int*)(d_s_nation + tile_offset), n, selection_flags, num_tile_items);
  BlockPredAndEQ<int, BT, IPT>(n, 23, selection_flags, num_tile_items);
  BlockLoadSelect<int, BT, IPT>((int*)(d_s_city_enc + tile_offset), c, selection_flags, num_tile_items);
  #pragma unroll
  for (int i = 0; i < IPT; ++i) {
    if ((threadIdx.x + BT * i) >= num_tile_items) break;
    if (!selection_flags[i]) continue;
    gpu_ht_insert_one_direct(k[i], c[i], 1, 1, d_ht, ht_size);
    gpu_bloom_set_one(k[i], 1, d_bf_bits, bf_size_bits, num_hashes);
  }
}

// part: p_category == 3 (MFGR#14), payload = p_brand1
template <int BT, int IPT>
__global__ void build_part(
    int num_tuples,
    const int *__restrict__ d_p_partkey, const int *__restrict__ d_p_category, const int *__restrict__ d_p_brand1,
    HtEntry *d_ht, uint32_t ht_size,
    uint64_t *__restrict__ d_bf_bits, uint32_t bf_size_bits, uint8_t num_hashes,
    const uint32_t *d_prefilter, const uint8_t *d_tile_summary)
{
  FASCAL_BUILD_KERNEL_PROLOGUE(BT, IPT);
  int k[IPT], c[IPT], b[IPT];
  BlockLoadSelect<int, BT, IPT>((int*)(d_p_partkey + tile_offset), k, selection_flags, num_tile_items);
  BlockLoadSelect<int, BT, IPT>((int*)(d_p_category + tile_offset), c, selection_flags, num_tile_items);
  BlockPredAndEQ<int, BT, IPT>(c, 3, selection_flags, num_tile_items);
  BlockLoadSelect<int, BT, IPT>((int*)(d_p_brand1 + tile_offset), b, selection_flags, num_tile_items);
  #pragma unroll
  for (int i = 0; i < IPT; ++i) {
    if ((threadIdx.x + BT * i) >= num_tile_items) break;
    if (!selection_flags[i]) continue;
    gpu_ht_insert_one_direct(k[i], b[i], 1, 1, d_ht, ht_size);
    gpu_bloom_set_one(k[i], 1, d_bf_bits, bf_size_bits, num_hashes);
  }
}
...
int main(int argc, char** argv) {
  ...
  LOAD_DIM(h_p_category, "p_category", num_part)
  LOAD_DIM(h_p_brand1, "p_brand1", num_part)
  ...
  cqo_in.predicate_selectivities = {
    fascal_estimate_range_sel(h_lo_orderdate, num_tuples, 19970101, 19981231)
  };
  ...
  int part_qual = (int)(fascal_estimate_eq_sel(h_p_category, num_part, 3) * num_part);
  ...
  DIM_ODZC(s_nation, h_s_nation, num_supp, "s_nation")
  DIM_ODZC(s_city_enc, h_s_city_enc, num_supp, "s_city_enc")
  DIM_ODZC(p_partkey, h_p_partkey, num_part, "p_partkey")
  DIM_ODZC(p_category, h_p_category, num_part, "p_category")
  DIM_ODZC(p_brand1, h_p_brand1, num_part, "p_brand1")
  #undef DIM_ODZC

  CUDA_CHECK(cudaMemcpyToSymbol(d_pred_flags, h_pred_flags, sizeof(h_pred_flags)));

  // Aggregation table: year(10) x city(250) x brand(1000)
  size_t num_groups = 10ULL * 250ULL * 1000ULL;
  ...
  // Customer build (no predicate — all rows qualify)
  ...
  // Supplier build (s_nation == 23, payload = s_city_enc)
  ...
  auto run_kernel = [&]() {
    ...
    auto launch = [&](int nt, int bo, int nb, cudaStream_t st) {
      ssb_q43_kernel<BLOCK_THREADS, ITEMS_PER_THREAD><<<nb, BLOCK_THREADS, 0, st>>>(
  ...
  for (size_t k = 0; k < num_groups; k++) {
    if (h_agg[k] != 0) {
      ...
      int city = (int)(gk % 250ULL); gk /= 250;
      int brand = (int)(gk % 1000ULL);
      printf("ROW: %d|%d|%d|%llu\n", year_off, city, brand, (unsigned long long)h_agg[k]);
    }
  }
  free(h_agg);
  ...
  odzc->unregister_column(mc_c_custkey);
  odzc->unregister_column(mc_s_suppkey);    odzc->unregister_column(mc_s_nation);
  odzc->unregister_column(mc_s_city_enc);   odzc->unregister_column(mc_p_partkey);
  odzc->unregister_column(mc_p_category);   odzc->unregister_column(mc_p_brand1);
  ...
  fascal_free_column(h_s_suppkey); fascal_free_column(h_s_nation); fascal_free_column(h_s_city_enc);
  fascal_free_column(h_p_partkey); fascal_free_column(h_p_category); fascal_free_column(h_p_brand1);
