// SSB Q4.2 -- FaScalSQL optimized kernel (ODZC + AFP + CQO)
// Refactored: uses shared macros and CPU predicate helpers.
// Dims: customer(c_region=AMERICA), supplier(s_region=AMERICA, payload=s_nation_enc), part(p_mfgr IN (MFGR#1,MFGR#2), payload=p_category_enc)
// Date: 1997-1998, GROUP BY d_year(inline) x s_nation_enc x p_category_enc, AGG SUM(lo_revenue - lo_supplycost)
...
__global__ void ssb_q42_kernel(
    ...
    unsigned long long *aggtable)
{
  ...
  int lo_revenue[IPT], lo_supplycost[IPT], s_nation_enc[IPT], p_category_enc[IPT];
  ...
  if (FASCAL_ANY_ALIVE(IPT)) do {
    ...
    BlockJoinProbePayloadDirect<BT, IPT>(lo_suppkey, selection_flags, num_tile_items, ht_supplier, ht_size_supplier, 1, s_nation_enc);
    BlockJoinProbePayloadDirect<BT, IPT>(lo_partkey, selection_flags, num_tile_items, ht_part, ht_size_part, 1, p_category_enc);
    ...
    // GROUP BY year(10) x nation(25) x category(25) -> profit
    #pragma unroll
    for (int i = 0; i < IPT; ++i)
      if ((threadIdx.x + BT * i) < num_tile_items && selection_flags[i]) {
        int gk = (lo_orderdate[i] / 10000 - 1992) + s_nation_enc[i] * 10 + p_category_enc[i] * 250;
...
// customer: c_region == 1 (AMERICA), no payload
template <int BT, int IPT>
__global__ void build_customer(
    int num_tuples,
    const int *__restrict__ d_c_custkey, const int *__restrict__ d_c_region,
...
// supplier: s_region == 1 (AMERICA), payload = s_nation_enc
template <int BT, int IPT>
__global__ void build_supplier(
    int num_tuples,
    const int *__restrict__ d_s_suppkey, const int *__restrict__ d_s_region, const int *__restrict__ d_s_nation_enc,
    HtEntry *d_ht, uint32_t ht_size,
    uint64_t *__restrict__ d_bf_bits, uint32_t bf_size_bits, uint8_t num_hashes,
    const uint32_t *d_prefilter, const uint8_t *d_tile_summary)
{
  ...
  BlockPredAndEQ<int, BT, IPT>(r, 1, selection_flags, num_tile_items);
  BlockLoadSelect<int, BT, IPT>((int*)(d_s_nation_enc + tile_offset), n, selection_flags, num_tile_items);
  #pragma unroll
  for (int i = 0; i < IPT; ++i) {
    if ((threadIdx.x + BT * i) >= num_tile_items) break;
    if (!selection_flags[i]) continue;
...
// part: p_mfgr == 0 OR p_mfgr == 1, payload = p_category_enc
template <int BT, int IPT>
__global__ void build_part(
    int num_tuples,
    const int *__restrict__ d_p_partkey, const int *__restrict__ d_p_mfgr, const int *__restrict__ d_p_category_enc,
    HtEntry *d_ht, uint32_t ht_size,
    uint64_t *__restrict__ d_bf_bits, uint32_t bf_size_bits, uint8_t num_hashes,
    const uint32_t *d_prefilter, const uint8_t *d_tile_summary)
{
  FASCAL_BUILD_KERNEL_PROLOGUE(BT, IPT);
  int k[IPT], m[IPT], c[IPT];
  ...
  BlockLoadSelect<int, BT, IPT>((int*)(d_p_category_enc + tile_offset), c, selection_flags, num_tile_items);
  #pragma unroll
  for (int i = 0; i < IPT; ++i) {
    if ((threadIdx.x + BT * i) >= num_tile_items) break;
    if (!selection_flags[i]) continue;
...
int main(int argc, char** argv) {
  ...
  DIM_ODZC(s_nation_enc, h_s_nation_enc, num_supp, "s_nation_enc")
  DIM_ODZC(p_partkey, h_p_partkey, num_part, "p_partkey")
  DIM_ODZC(p_mfgr, h_p_mfgr, num_part, "p_mfgr")
  DIM_ODZC(p_category_enc, h_p_category_enc, num_part, "p_category_enc")
  #undef DIM_ODZC

  CUDA_CHECK(cudaMemcpyToSymbol(d_pred_flags, h_pred_flags, sizeof(h_pred_flags)));

  // Aggregation table: year(10) x nation(25) x category(25)
  size_t num_groups = 10ULL * 25ULL * 25ULL;
  ...
  build_customer<BLOCK_THREADS, ITEMS_PER_THREAD><<<(num_cust + TILE_SIZE - 1) / TILE_SIZE, BLOCK_THREADS>>>(
    num_cust, d_c_custkey, d_c_region,
  ...
  // Supplier build (s_region == 1, payload = s_nation_enc)
  ...
  build_supplier<BLOCK_THREADS, ITEMS_PER_THREAD><<<(num_supp + TILE_SIZE - 1) / TILE_SIZE, BLOCK_THREADS>>>(
    num_supp, d_s_suppkey, d_s_region, d_s_nation_enc,
  ...
  // Part build (p_mfgr IN (0,1), payload = p_category_enc)
  ...
  build_part<BLOCK_THREADS, ITEMS_PER_THREAD><<<(num_part + TILE_SIZE - 1) / TILE_SIZE, BLOCK_THREADS>>>(
    num_part, d_p_partkey, d_p_mfgr, d_p_category_enc,
  ...
  auto run_kernel = [&]() {
    ...
    auto launch = [&](int nt, int bo, int nb, cudaStream_t st) {
      ssb_q42_kernel<BLOCK_THREADS, ITEMS_PER_THREAD><<<nb, BLOCK_THREADS, 0, st>>>(
  ...
  for (size_t k = 0; k < num_groups; k++) {
    if (h_agg[k] != 0) {
      ...
      int nation = (int)(gk % 25ULL); gk /= 25;
      int category = (int)(gk % 25ULL);
      printf("ROW: %d|%d|%d|%llu\n", year_off, nation, category, (unsigned long long)h_agg[k]);
    }
  }
  free(h_agg);
  ...
  odzc->unregister_column(mc_s_nation_enc); odzc->unregister_column(mc_p_partkey);
  odzc->unregister_column(mc_p_mfgr);      odzc->unregister_column(mc_p_category_enc);
  ...
  fascal_free_column(h_c_custkey); fascal_free_column(h_c_region);
  fascal_free_column(h_s_suppkey); fascal_free_column(h_s_region); fascal_free_column(h_s_nation_enc);
  fascal_free_column(h_p_partkey); fascal_free_column(h_p_mfgr);  fascal_free_column(h_p_category_enc);
