// SSB Q2.2 -- FaScalSQL optimized kernel (ODZC + AFP + CQO)
// Refactored: uses shared macros and CPU predicate helpers.
// Part dim: p_brand1 BETWEEN 253 AND 260 (MFGR#2221..MFGR#2228), payload=p_brand1.
// Supplier dim: s_region=2 (ASIA). Date join eliminated: inline year.
...
__global__ void ssb_q22_kernel(
...
// =============================================================================
// Build kernel: part dimension (p_brand1 BETWEEN 253 AND 260, payload=p_brand1)
// =============================================================================
template <int BT, int IPT>
    ...
    const uint32_t *d_prefilter, const uint8_t *d_tile_summary)
{
  ...
  BlockPredAndGTE<int, BT, IPT>(br, 253, selection_flags, num_tile_items);
  BlockPredAndLTE<int, BT, IPT>(br, 260, selection_flags, num_tile_items);
...
// =============================================================================
// Build kernel: supplier dimension (s_region=2, existence HT)
// =============================================================================
template <int BT, int IPT>
    ...
    const uint32_t *d_prefilter, const uint8_t *d_tile_summary)
{
  ...
  BlockPredAndEQ<int, BT, IPT>(sr, 2, selection_flags, num_tile_items);
...
static void cpu_prefilter_part(int *h_pk, int *h_br, int nt, uint32_t *bm, uint8_t *ts) {
  fascal_cpu_prefilter_run(nt, bm, ts, [&](int off, int cnt) {
    ...
    for (int i = 0; i < cnt; ++i) { int idx = off + i;
      if (h_br[idx] >= 253 && h_br[idx] <= 260) bm[(uint32_t)idx >> 5] |= (1u << ((uint32_t)idx & 31));
    }
  });
}
...
static void cpu_prefilter_supp(int *h_sk, int *h_sr, int nt, uint32_t *bm, uint8_t *ts) {
  fascal_cpu_prefilter_run(nt, bm, ts, [&](int off, int cnt) {
    ...
    memset(&bm[ws], 0, (we - ws) * sizeof(uint32_t));
    for (int i = 0; i < cnt; ++i) { int idx = off + i;
      if (h_sr[idx] == 2) bm[(uint32_t)idx >> 5] |= (1u << ((uint32_t)idx & 31));
    }
  });
}
...
int main(int argc, char** argv) {
  ...
  int part_qual = (int)(fascal_estimate_range_sel(h_p_brand1, num_part, 253, 260) * num_part);
  ...
  auto run_kernel = [&]() {
    ...
    auto launch = [&](int nt, int bo, int nb, cudaStream_t st) {
      ssb_q22_kernel<BLOCK_THREADS, ITEMS_PER_THREAD><<<nb, BLOCK_THREADS, 0, st>>>(
