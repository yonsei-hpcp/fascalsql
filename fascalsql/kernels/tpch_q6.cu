// TPC-H Q6 -- FaScalSQL optimized kernel (ODZC + AFP + CQO)
...
// =============================================================================
// GPU Kernel: scan lineitem, apply predicates, aggregate
// =============================================================================
template <int BT, int IPT>
__global__ void tpch_q6_kernel(
    int num_tuples, int batch_offset,
    int *d_l_shipdate, float *d_l_quantity, float *d_l_discount, float *d_l_extendedprice,
    ...
    double *aggtable)
{
  ...
  int l_shipdate[IPT];
  float l_quantity[IPT], l_discount[IPT], l_extendedprice[IPT];

  if (FASCAL_ANY_ALIVE(IPT)) do {
    // Pred 3: l_discount BETWEEN 0.07 AND 0.09
    BlockLoadSelect<float, BT, IPT>(d_l_discount + tile_offset, l_discount, selection_flags, num_tile_items);
    if (d_pred_flags[3] == 0) {
      BlockPredAndGTE<float, BT, IPT>(l_discount, 0.07f, selection_flags, num_tile_items);
      BlockPredAndLTE<float, BT, IPT>(l_discount, 0.09f, selection_flags, num_tile_items);
    }

    // Pred 0,1: l_shipdate >= 19940101 AND l_shipdate < 19950101
    if (d_pred_flags[0] == 0 || d_pred_flags[1] == 0) {
      ...
      if (d_pred_flags[0] == 0)
        BlockPredAndGTE<int, BT, IPT>(l_shipdate, 19940101, selection_flags, num_tile_items);
      if (d_pred_flags[1] == 0)
        BlockPredAndLT<int, BT, IPT>(l_shipdate, 19950101, selection_flags, num_tile_items);
    }

    // Pred 2: l_quantity < 24
    if (d_pred_flags[2] == 0) {
      BlockLoadSelect<float, BT, IPT>(d_l_quantity + tile_offset, l_quantity, selection_flags, num_tile_items);
      BlockPredAndLT<float, BT, IPT>(l_quantity, 24, selection_flags, num_tile_items);
    }
  ...
  for (int i = 0; i < IPT; ++i)
    if ((threadIdx.x + BT * i) < num_tile_items && selection_flags[i])
      acc += (double)l_extendedprice[i] * l_discount[i];
  acc = blockReduceSumDouble<BT>(acc);
  if (threadIdx.x == 0) atomicAdd(&aggtable[0], acc);
}
...
static void cpu_predicate(int *h_data[], int offset, int cnt, uint32_t *bitmap) {
  ...
  float *l_quantity = (float*)h_data[1];
  float *l_discount = (float*)h_data[2];

  // Pass 0: l_shipdate >= 19940101 AND l_shipdate < 19950101
  if (h_pred_flags[0] == 1 || h_pred_flags[1] == 1)
    fascal::cpu_pred::avx2_range_pass(bm, bs, l_shipdate, offset, cnt,
        19940101, 19950101, h_pred_flags[0] == 1, h_pred_flags[1] == 1, true);

  // Pass 1: l_quantity < 24 (float -- hand-written AVX2)
  if (h_pred_flags[2] == 1) {
    for (int i = 0; i + 7 < cnt; i += 8) {
      int bidx = bs + (i >> 3);
      if (!bm[bidx]) continue;
      __m256 v = _mm256_loadu_ps(l_quantity + offset + i);
      __m256i p = _mm256_castps_si256(_mm256_cmp_ps(v, _mm256_set1_ps(24.0f), _CMP_LT_OS));
      bm[bidx] &= (uint8_t)_mm256_movemask_ps(_mm256_castsi256_ps(p));
    }
    int tail_start = cnt & ~7;
    if (tail_start < cnt) {
      int bidx = bs + (tail_start >> 3);
      uint8_t bv = bm[bidx];
      if (bv) {
        for (int i = tail_start; i < cnt; ++i) {
          int bit = i & 7;
          if ((bv & (1u << bit)) && !(l_quantity[offset + i] < 24.0f))
            bv &= ~(1u << bit);
        }
        bm[bidx] = bv;
      }
  ...
  // Pass 2: l_discount BETWEEN 0.07 AND 0.09 (float -- hand-written AVX2)
  if (h_pred_flags[3] == 1) {
    for (int i = 0; i + 7 < cnt; i += 8) {
      int bidx = bs + (i >> 3);
      if (!bm[bidx]) continue;
      __m256 v = _mm256_loadu_ps(l_discount + offset + i);
      __m256 ge = _mm256_cmp_ps(v, _mm256_set1_ps(0.07f), _CMP_GE_OS);
      __m256 le = _mm256_cmp_ps(v, _mm256_set1_ps(0.09f), _CMP_LE_OS);
      __m256i p = _mm256_castps_si256(_mm256_and_ps(ge, le));
      bm[bidx] &= (uint8_t)_mm256_movemask_ps(_mm256_castsi256_ps(p));
    }
    int tail_start = cnt & ~7;
    if (tail_start < cnt) {
      ...
      if (bv) {
        for (int i = tail_start; i < cnt; ++i) {
          int bit = i & 7;
          if ((bv & (1u << bit)) && !(l_discount[offset + i] >= 0.07f && l_discount[offset + i] <= 0.09f))
            bv &= ~(1u << bit);
        }
        bm[bidx] = bv;
      }
...
int main(int argc, char** argv) {
  ...
  // CQO — sample-based selectivity estimation (paper §IV-D)
  // Q6: WHERE l_shipdate >= 19940101 AND l_shipdate < 19950101
  //          AND l_discount BETWEEN 5 AND 7  AND l_quantity < 24
  auto cqo_in = fascal_cqo_init(num_tuples);
  cqo_in.predicate_selectivities = {
    fascal_estimate_range_sel(h_l_shipdate, num_tuples, 19940101, 19941231),
    fascal_estimate_range_sel(h_l_shipdate, num_tuples, 19940101, 19950100),  // < 19950101
    fascal_estimate_range_sel(h_l_discount, num_tuples, 5, 7),
    fascal_estimate_selectivity(h_l_quantity, num_tuples, [](int v){ return v < 24; })
  };
  ...
  // Aggregation table
  double *d_agg;
  CUDA_CHECK(cudaMalloc(&d_agg, NUM_AGGREGATES * sizeof(double)));
  CUDA_CHECK(cudaMemset(d_agg, 0, NUM_AGGREGATES * sizeof(double)));

  // Pipeline setup
  int *h_data[] = {h_l_shipdate, (int*)h_l_quantity, (int*)h_l_discount, (int*)h_l_extendedprice};
  ...
  auto run_kernel = [&]() {
    ...
    auto launch = [&](int nt, int bo, int nb, cudaStream_t st) {
      tpch_q6_kernel<BLOCK_THREADS, ITEMS_PER_THREAD><<<nb, BLOCK_THREADS, 0, st>>>(
        nt, bo, d_l_shipdate, d_l_quantity, d_l_discount, d_l_extendedprice, d_bm, d_ts, d_agg);
    };
  ...
  fascal_free_column(h_l_shipdate);  fascal_free_column(h_l_quantity);
  fascal_free_column(h_l_discount);  fascal_free_column(h_l_extendedprice);
