// SSB Q3.3 -- FaScalSQL optimized kernel (ODZC + AFP + CQO)
...
__global__ void ssb_q33_kernel(
    ...
    unsigned long long *aggtable)
{
  ...
  if (FASCAL_ANY_ALIVE(IPT)) do {
    ...
    // Pred 4: lo_orderdate BETWEEN 19920101 AND 19971231
    if (d_pred_flags[4] == 0) {
...
int main(int argc, char** argv) {
  ...
  auto run_kernel = [&]() {
    ...
    auto launch = [&](int nt, int bo, int nb, cudaStream_t st) {
      ssb_q33_kernel<BLOCK_THREADS, ITEMS_PER_THREAD><<<nb, BLOCK_THREADS, 0, st>>>(
