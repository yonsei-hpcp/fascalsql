// SSB Q1.1 -- FaScalSQL optimized kernel (ODZC + AFP + CQO)
    ...
    unsigned long long *aggtable)
{
  ...
  if (FASCAL_ANY_ALIVE(IPT)) do {
    // Pred 0: lo_discount BETWEEN 1 AND 3
    BlockLoadSelect<int, BT, IPT>(d_lo_discount + tile_offset, lo_discount, selection_flags, num_tile_items);
    if (d_pred_flags[0] == 0) {
      BlockPredAndGTE<int, BT, IPT>(lo_discount, 1, selection_flags, num_tile_items);
      BlockPredAndLTE<int, BT, IPT>(lo_discount, 3, selection_flags, num_tile_items);
    }

    // Pred 1: lo_quantity < 25
    if (d_pred_flags[1] == 0) {
      BlockLoadSelect<int, BT, IPT>(d_lo_quantity + tile_offset, lo_quantity, selection_flags, num_tile_items);
      BlockPredAndLT<int, BT, IPT>(lo_quantity, 25, selection_flags, num_tile_items);
    }

    // Pred 2: lo_orderdate BETWEEN 19930101 AND 19931231
    if (d_pred_flags[2] == 0) {
      BlockLoadSelect<int, BT, IPT>(d_lo_orderdate + tile_offset, lo_orderdate, selection_flags, num_tile_items);
      BlockPredAndGTE<int, BT, IPT>(lo_orderdate, 19930101, selection_flags, num_tile_items);
      BlockPredAndLTE<int, BT, IPT>(lo_orderdate, 19931231, selection_flags, num_tile_items);
    }
...
static void cpu_predicate(int *h_data[], int offset, int cnt, uint32_t *bitmap) {
  ...
  if (h_pred_flags[0] == 1)
    fascal::cpu_pred::avx2_range_pass(bm, bs, lo_discount, offset, cnt, 1, 3, true, true);
  if (h_pred_flags[1] == 1)
    fascal::cpu_pred::avx2_range_pass(bm, bs, lo_quantity, offset, cnt, 0, 25, false, true, true);
  if (h_pred_flags[2] == 1)
    fascal::cpu_pred::avx2_range_pass(bm, bs, lo_orderdate, offset, cnt, 19930101, 19931231, true, true);
}
...
int main(int argc, char** argv) {
  ...
  cqo_in.predicate_selectivities = {
    fascal_estimate_range_sel(h_lo_discount, num_tuples, 1, 3),
    fascal_estimate_selectivity(h_lo_quantity, num_tuples, [](int v){ return v < 25; }),
    fascal_estimate_range_sel(h_lo_orderdate, num_tuples, 19930101, 19931231)
  };
  ...
  auto run_kernel = [&]() {
    ...
    auto launch = [&](int nt, int bo, int nb, cudaStream_t st) {
      ssb_q11_kernel<BLOCK_THREADS, ITEMS_PER_THREAD><<<nb, BLOCK_THREADS, 0, st>>>(
