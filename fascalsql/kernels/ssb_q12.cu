// SSB Q1.2 -- FaScalSQL optimized kernel (ODZC + AFP + CQO)
...
__global__ void ssb_q12_kernel(
    ...
    unsigned long long *aggtable)
{
  ...
  if (FASCAL_ANY_ALIVE(IPT)) do {
    // Pred 0: lo_discount BETWEEN 4 AND 6
    BlockLoadSelect<int, BT, IPT>(d_lo_discount + tile_offset, lo_discount, selection_flags, num_tile_items);
    if (d_pred_flags[0] == 0) {
      BlockPredAndGTE<int, BT, IPT>(lo_discount, 4, selection_flags, num_tile_items);
      BlockPredAndLTE<int, BT, IPT>(lo_discount, 6, selection_flags, num_tile_items);
    }
    ...
    // Pred 2: lo_orderdate BETWEEN 19940101 AND 19940131
    if (d_pred_flags[2] == 0) {
      BlockLoadSelect<int, BT, IPT>(d_lo_orderdate + tile_offset, lo_orderdate, selection_flags, num_tile_items);
      BlockPredAndGTE<int, BT, IPT>(lo_orderdate, 19940101, selection_flags, num_tile_items);
      BlockPredAndLTE<int, BT, IPT>(lo_orderdate, 19940131, selection_flags, num_tile_items);
    }
...
static void cpu_predicate(int *h_data[], int offset, int cnt, uint32_t *bitmap) {
  ...
  if (h_pred_flags[0] == 1)
    fascal::cpu_pred::avx2_range_pass(bm, bs, lo_discount, offset, cnt, 4, 6, true, true);
  if (h_pred_flags[1] == 1)
    fascal::cpu_pred::avx2_range_pass(bm, bs, lo_quantity, offset, cnt, 26, 35, true, true);
  if (h_pred_flags[2] == 1)
    fascal::cpu_pred::avx2_range_pass(bm, bs, lo_orderdate, offset, cnt, 19940101, 19940131, true, true);
}
...
int main(int argc, char** argv) {
  ...
  cqo_in.predicate_selectivities = {
    fascal_estimate_range_sel(h_lo_discount, num_tuples, 4, 6),
    fascal_estimate_range_sel(h_lo_quantity, num_tuples, 26, 35),
    fascal_estimate_range_sel(h_lo_orderdate, num_tuples, 19940101, 19940131)
  };
  ...
  auto run_kernel = [&]() {
    ...
    auto launch = [&](int nt, int bo, int nb, cudaStream_t st) {
      ssb_q12_kernel<BLOCK_THREADS, ITEMS_PER_THREAD><<<nb, BLOCK_THREADS, 0, st>>>(
