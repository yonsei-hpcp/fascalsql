// SSB Q3.4 -- FaScalSQL optimized kernel (ODZC + AFP + CQO)
// Refactored: uses shared macros and CPU predicate helpers.
// Dimensions: customer (c_city IN (221,225) -> c_city_enc), supplier (s_city IN (221,225) -> s_city_enc)
// Date filter: lo_orderdate BETWEEN 19971201 AND 19971231 (d_yearmonth = 'Dec1997')
...
// OR-group normalization: preds 0,1 (c_city) and 2,3 (s_city) must be placed together
...
__global__ void ssb_q34_kernel(
    ...
    unsigned long long *aggtable)
{
  ...
  if (FASCAL_ANY_ALIVE(IPT)) do {
    ...
    // Pred 4: lo_orderdate BETWEEN 19971201 AND 19971231 (Dec 1997)
    if (d_pred_flags[4] == 0) {
      BlockLoadSelect<int, BT, IPT>(d_lo_orderdate + tile_offset, lo_orderdate, selection_flags, num_tile_items);
      BlockPredAndGTE<int, BT, IPT>(lo_orderdate, 19971201, selection_flags, num_tile_items);
...
// =============================================================================
// Build kernel: customer (c_city IN (221,225) -- OR predicate)
// =============================================================================
template <int BT, int IPT>
__global__ void build_customer(
    int num_tuples,
    const int *__restrict__ d_c_custkey, const int *__restrict__ d_c_city,
    ...
    const uint32_t *d_prefilter, const uint8_t *d_tile_summary)
{
  ...
  int key[IPT], city[IPT], payload[IPT];

  BlockLoadSelect<int, BT, IPT>((int*)(d_c_custkey + tile_offset), key, selection_flags, num_tile_items);
  BlockLoadSelect<int, BT, IPT>((int*)(d_c_city + tile_offset), city, selection_flags, num_tile_items);

  // OR predicate: c_city == 221 OR c_city == 225
  #pragma unroll
  for (int i = 0; i < IPT; ++i)
    if ((threadIdx.x + BT * i) < num_tile_items && selection_flags[i])
      selection_flags[i] = (city[i] == 221 || city[i] == 225) ? 1 : 0;
...
// =============================================================================
// Build kernel: supplier (s_city IN (221,225) -- OR predicate)
// =============================================================================
template <int BT, int IPT>
__global__ void build_supplier(
    int num_tuples,
    const int *__restrict__ d_s_suppkey, const int *__restrict__ d_s_city,
    ...
    const uint32_t *d_prefilter, const uint8_t *d_tile_summary)
{
  ...
  BlockLoadSelect<int, BT, IPT>((int*)(d_s_city + tile_offset), city, selection_flags, num_tile_items);

  // OR predicate: s_city == 221 OR s_city == 225
  #pragma unroll
  for (int i = 0; i < IPT; ++i)
    if ((threadIdx.x + BT * i) < num_tile_items && selection_flags[i])
      selection_flags[i] = (city[i] == 221 || city[i] == 225) ? 1 : 0;
...
// =============================================================================
// CPU Predicate: date range (Dec 1997) + 2 BF probes
// =============================================================================
static void cpu_predicate(int *h_data[], int offset, int cnt, uint32_t *bitmap,
  ...
  if (h_pred_flags[4] == 1)
    fascal::cpu_pred::avx2_range_pass(bm, bs, lo_orderdate, offset, cnt, 19971201, 19971231, true, true);
...
int main(int argc, char** argv) {
  ...
  LOAD_DIM(h_c_city, "c_city", num_cust)
  ...
  LOAD_DIM(h_s_city, "s_city", num_supp)
  ...
  cqo_in.predicate_selectivities = {0.4, 0.4, 0.4, 0.4,
    fascal_estimate_range_sel(h_lo_orderdate, num_tuples, 19971201, 19971231)
  };
  int cust_qual = (int)(fascal_estimate_selectivity(h_c_city, num_cust, [](int v){ return v == 221 || v == 225; }) * num_cust);
  int supp_qual = (int)(fascal_estimate_selectivity(h_s_city, num_supp, [](int v){ return v == 221 || v == 225; }) * num_supp);
  ...
  // CPU prefilter for build: customer (c_city IN (221,225))
  fascal_cpu_prefilter_run(num_cust, h_cust_pf, h_cust_ts,
      [&](int off, int cnt) {
          ...
          for (int i = 0; i < cnt; ++i) { int idx = off + i;
            if (h_c_city[idx] == 221 || h_c_city[idx] == 225) h_cust_pf[idx >> 5] |= (1u << (idx & 31)); }
      });

  // CPU prefilter for build: supplier (s_city IN (221,225))
  fascal_cpu_prefilter_run(num_supp, h_supp_pf, h_supp_ts,
      [&](int off, int cnt) {
          uint32_t ws = (uint32_t)off >> 5, we = ((uint32_t)(off + cnt) + 31) >> 5;
          memset(&h_supp_pf[ws], 0, (we - ws) * sizeof(uint32_t));
          for (int i = 0; i < cnt; ++i) { int idx = off + i;
            if (h_s_city[idx] == 221 || h_s_city[idx] == 225) h_supp_pf[idx >> 5] |= (1u << (idx & 31)); }
      });
  ...
  build_customer<BLOCK_THREADS, ITEMS_PER_THREAD><<<(num_cust + TILE_SIZE - 1) / TILE_SIZE, BLOCK_THREADS>>>(
      num_cust, d_c_custkey, d_c_city, d_c_city_enc,
  ...
  build_supplier<BLOCK_THREADS, ITEMS_PER_THREAD><<<(num_supp + TILE_SIZE - 1) / TILE_SIZE, BLOCK_THREADS>>>(
      num_supp, d_s_suppkey, d_s_city, d_s_city_enc,
  ...
  auto run_kernel = [&]() {
    ...
    auto launch = [&](int nt, int bo, int nb, cudaStream_t st) {
      ssb_q34_kernel<BLOCK_THREADS, ITEMS_PER_THREAD><<<nb, BLOCK_THREADS, 0, st>>>(
  ...
  odzc->unregister_column(mc_c_custkey); odzc->unregister_column(mc_c_city);
  odzc->unregister_column(mc_c_city_enc);
  odzc->unregister_column(mc_s_suppkey); odzc->unregister_column(mc_s_city);
